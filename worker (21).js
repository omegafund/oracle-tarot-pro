...
// [V27.0.2] 사장님 안 100% 반영 — CORE+TAIL 분리 / 행동형 TAIL / 익절-매도 분리
//   ① 가드 안전화: includes 패턴 (향후 도메인 확장 시 배열만 추가)
//   ② CORE+TAIL 분리: CORE(공통 톤) + TAIL(자산별 위험 경고)
//   ③ 익절(타이밍 게임) vs 매도(실행 단계) 정밀 분리
//   ④ '결과가 갈린다' 계열 어휘 (분기점/갈리는/달라지는/직결되는)
//   ⑤ 절대 금지 단어 0건 (~좋습니다/~가능성/~추천)
function buildOneLineSummary(metrics) {
  ...
  // ── CORE 매트릭스 (사장님 '결과가 갈린다' 계열)
  const core = TLDR_CORE_MATRIX[scenarioKey] || TLDR_CORE_MATRIX.wait_buy;
  // ── TAIL 매트릭스 (자산별 행동형 위험 경고)
  const tail = ASSET_TAIL_MATRIX[metrics.queryType] || ASSET_TAIL_MATRIX.stock;
...
function buildRealEstateOneLineSummary(metrics) {
  ...
  const tail = ASSET_TAIL_MATRIX.realestate;

  decision.oneLineSummary = `${core}\n${tail}`;
  return metrics;
}

// [V27.0.2] TLDR (한줄 결론) CORE 매트릭스
//   사장님 안 100% — '결과가 갈린다' 계열 강화
//   톤: [흐름 인정] + [, but] + [분기/결정 어휘]
//   킬러 단어: 분기점 / 갈리는 / 달라지는 / 직결되는 / 결정 단계
//   금지 단어: ~좋습니다 / ~가능성이 있습니다 / ~추천 (0건)
const TLDR_CORE_MATRIX = {
  // 주식·코인 6개 시나리오
  wait_buy:  '진입 흐름은 열려 있지만, 지금 구간은 타이밍에 따라 손익이 갈리는 분기점입니다',
  wait_sell: '익절 흐름은 살아있지만, 청산 타이밍에 따라 수익률이 갈리는 결정 단계입니다',
  verified:  '진입 신호는 형성됐지만, 분할 여부에 따라 수익 구조가 달라지는 구간입니다',
  limited:   '기회는 존재하지만, 신호 확인 없는 진입은 손실로 직결되는 구간입니다',
  active:    '결정은 명확하지만, 실행 시점에 따라 결과가 갈리는 분기점입니다',
  split:     '분할 진입은 유효하지만, 단계별 점검에 따라 수익 구조가 달라지는 구간입니다',
  // 부동산 6개 시나리오
  re_wait_buy:      '매수 기회는 존재하지만, 입지 선택에 따라 장기 수익이 갈리는 분기점입니다',
  re_wait_sell_act: '매도 흐름은 열려 있지만, 시점 선택에 따라 가격이 갈리는 결정 단계입니다',
  re_wait_sell_pas: '매도 흐름은 형성 중이지만, 매수자 유입 시점에 따라 결과가 달라지는 구간입니다',
  re_verified:      '진입 신호는 형성됐지만, 입지·자금 점검에 따라 장기 수익이 갈리는 구간입니다',
  re_active:        '결정은 명확하지만, 입지 검증에 따라 장기 결과가 갈리는 분기점입니다',
  re_holding:       '보유 흐름은 유효하지만, 시장 흐름 재평가에 따라 자산 가치가 달라지는 구간입니다'
};

// [V27.0.2] TAIL 매트릭스 — 사장님 안: 행동형 위험 경고
//   설명형 ❌ ("~핵심입니다") → 행동형 ✓ ("~없이 진입하면 ~됩니다")
//   본질: FOMO + 손실 회피 심리 자극 → 결제 전환 ↑
const ASSET_TAIL_MATRIX = {
  stock:      '→ 거래량 확인 없이 진입하면 변동성에 노출됩니다',
  crypto:     '→ 손절 기준 없이 진입하면 손실 누적 위험이 큽니다',
  realestate: '→ 입지 검증 없이 진입하면 장기 묶임 가능성이 높습니다'
};
