...
function getCardSentence(card, isReversed, position, promptText) {
  ...
  if (pool && pool[position]) {
    ...
    return isReversed ? `(역방향) ${base}` : base;
  }

  // 폴백: CARD_TAG 기반 자동 생성
  const tags = getCardTags(card, isReversed);
  const t0 = tags[0];
  if (position === 'past')    return `과거에는 ${t0}의 흐름이 에너지의 기반을 만들었습니다.`;
  if (position === 'present') return `현재 흐름의 중심에는 ${t0}가 작동하고 있습니다.`;
  return `앞으로는 ${t0}의 흐름이 방향을 결정하게 됩니다.`;
}
