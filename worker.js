...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/version" && request.method === "GET") {
      return new Response(JSON.stringify({
        version: "V202.52.1",    // ★ V202.52.1 핫픽스: (A) LOVE_SYNONYM_MAP 4단어 추가 [단계/정리/회복/종결] — 라이브 반복 9~11회 차단, (B) 카드 본문 톤 매니페스토 #4 — 사장님 톤 정렬 (3요소: 부정전제+미묘한결+서브타입맥락) + 카테고리별 정서밀도 6단 차별화 [썸/연락/짝사랑/재회/결혼/이별] — 결제 가치 직결 핫픽스. V202.48 v2.0 base 위에 적용.
        _ts: Date.now(),
        _ok: true
      }), {
        headers: {
