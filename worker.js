        ...
        if (queryType === "stock" || queryType === "crypto") {
          ...
          function extractKoreanNames(p) {
            ...
            const _NOT_NAMES = new Set([
              ...
              // ★ V202.52.5 보강 ★ 1인칭 + 조사 (3순위 공백 매칭 오인 차단)
              '나와','나랑','나하고','나에게','저와','저랑','저하고','저에게',
              '너와','너랑','너하고','내게','네게',
              // ★ V202.52.5 보강 ★ 행동 명사/동사 (TC8 "지영이와 이별" 결함)
            ...
            const _isName = (s) => s && s.length >= 2 && s.length <= 4 && !_NOT_NAMES.has(s);

            let m;

            // 1순위: '이름1 + 와/과/랑/하고 + 이름2' (조사 결합 — 가장 명확)
            m = p.match(/([가-힣]{2,4})(?:님|씨)?(?:과|와|이랑|랑|하고)\s+([가-힣]{2,4})(?:님|씨)?/);
            if (m && _isName(m[1]) && _isName(m[2])) return [m[1], m[2]];
            ...
            if (m && _isName(m[1])) return [m[1]];

            // 5순위: '이름 + 님' (호칭형 단일)
            m = p.match(/([가-힣]{2,4})님(?:과|와|이랑|랑|하고|의|에게|에|이|가)/);
            if (m && _isName(m[1])) return [m[1]];

            // 6순위: '이름 + 와의/랑의/이와/이랑' (관계형 단일)
            //   예: "지영이와 이별", "지영이랑 다시"
