...
function applyCryptoVocabulary(metrics, stockSubType, stockIntent) {
  ...
  const VOCAB = {
    crypto_buy: {
      ...
      coreKey: '🟢 30~50% 분할 진입 + 손절 -7~-10% 기준 + 24시간 변동성 관리 핵심'
    },
    crypto_sell: {
      ...
      coreKey: '🔴 즉시 30~50% 분할 청산 + 변동성 확대 시 잔여 정리 + 욕심 차단 핵심'
    },
    scalping: {
...
const TLDR_CORE_MATRIX = {
  // ★★★ [V202.20 사장님 95점 포맷] ★★★
  //   주식·코인 6개 시나리오 — 색상 신호 + 수치 범위 + 구체 행동
  //   ① 🟢/🟡/🔴 색상 신호 (1초 가치 전달)
  //   ② 30~50%, 3~5일 (수치 범위)
  //   ③ "신호 확인 → 분할 진입" (구체 행동)
  //   주식·코인 6개 시나리오
  wait_buy:  '🟡 방향 형성 중 — 신호 확인 후 30~50% 분할 진입 권장 (확인 없이 일괄 진입 시 손실 직결)',
  wait_sell: '🔴 익절 적기 — 3~5일 분할 청산 30~50% 권장 (청산 지연 시 수익 반납 위험)',
  verified:  '🟢 진입 조건 충족 — 1주일 분할 30~50% 매수 가능 (풀 진입 시 변동성 직접 노출)',
  limited:   '🟡 제한 진입 — 10~20% 소액 테스트만 (본격 진입 시 추가 하락 가능성)',
  active:    '🟢 진입 활성 구간 — 1~3일 분할 30~50% (손절 -5~-7% 기준 필수)',
  split:     '🟢 분할 적기 — 3~5단계 (30%/20%/20%/15%/15%) (계획 없는 분할은 평균단가 망가짐)',
...
const STOCK_BLOCK_MATRIX = {
  ...
  wait_buy: {
    ...
    TURN: ['핵심:', '주의:', '실행 기준:'],
    RISK: [
      '확인 전 진입 시 손실 직결 (분할 0~10% 권장)',
      '검증 미충족 매수는 손익 분기점 (관망 우선)',
      '신호 부족 구간 — 진입 시 변동성 노출 위험',
      '확인 없는 진입 = 손실 전환 가능성 70% 이상',
      '신호 확인 전 진입 = 최대 리스크 구간'
    ],
  ...
  limited: {
    CORE: [
      '🟡 제한적 시도 구간 — 소액 테스트만',
      '🟡 진입 여지 일부 존재 — 신중 접근',
      '🟡 시장 신호 부분 발생 — 10~20% 한정',
      '🟡 소액 테스트 가능 단계',
      '🟡 부분 진입 여지 — 검증 우선'
    ],
    TURN: ['핵심:', '제한 기준:', '주의:'],
    RISK: [
      '본격 진입 X — 추가 하락 가능성 50% 이상',
      '소액 테스트 외 = 리스크 우위 구간',
      '확인 없는 진입 = 손실 직결 가능성 높음',
      '제한 없는 시도 = 변동성 확대 노출',
