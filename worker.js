...
function buildTopVerdict(metrics) {
  ...
  // [V203.12] 단타 스타일 — 총점으로 4가지 단타 시나리오 선택
  if (metrics.stockSubType === 'short' || metrics._tradingStyle === 'short') {
    const _ts = metrics.totalScore || 0;
    if (_ts >= 4)        key = 'short_breakout';  // 강한 상승 에너지 → 돌파
    else if (_ts >= 1)   key = 'short_pullback';  // 약한 상승 → 눌림목
    else if (_ts >= -2)  key = 'short_wait';      // 중립 → 관망
    else                 key = 'short_avoid';     // 하락 에너지 → 추격 금지
  }
...
[V203.12 단타 점사 전용 규칙 — stockSubType=short일 때만 적용]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
${stockSubType === 'short' ? `
📌 단타(short-term) 전용 해석 규칙:
  - 모든 카드 해석을 단기(당일~3일) 관점으로 작성
  - "장기 보유", "중장기", "안정적 가치" 표현 금지
  - 대신: "단기 모멘텀", "거래량", "당일 흐름", "돌파/눌림", "변동성" 사용
  - ZEUS ORACLE: 단타 타이밍 중심 3문장 (언제 진입? 어떤 신호? 손절 기준?)
  - 카드 해석 예시:
    Ten of Pentacles → "수급이 안정적이다, 급등보다 꾸준한 흐름 — 단타보다 스윙 적합"
    The Star → "단기 반등 모멘텀이 형성될 수 있는 구간"
    Four of Swords → "당일 관망, 에너지 응축 중 — 돌파 신호 대기"
` : ''}
