...
function buildCriticalInterpretation(cards, revFlags, domain, intent) {
  ...
  // ════════════════════════════════════════════════════════
  // [V25.9.3] 사장님 최종안 — 3단락 + 👉 + ※ 면책 통일 구조
  //   사장님 진단: "다른 영역(매수매도/코인/부동산/운세)에도 통일"
  //   해결: domain × intent × signal 매트릭스 → 3단락 흐름으로 통일
  //   구조: ① 시사·해석 ② 대응 가능성 ③ 👉 결론 + ※ 면책
  // ════════════════════════════════════════════════════════
  ...
  // ─ Paragraph 1: 시사·해석 (signal 기반) ─
  let para1;
  if (domain === "realestate") {
    if (intent === "sell") {
      para1 = signal === "SELL" ? `현재 부동산 카드 조합은 추세 지속보다는 흐름 재평가 가능성을 시사하는 구간으로 해석됩니다. 단순히 호가 유지로 결과를 기다리기보다는, 시장 변화에 따른 대응을 함께 고려할 필요가 있습니다.`
            : signal === "BUY"  ? `현재 부동산 카드 조합은 매도자에게 우호적 흐름 가능성을 시사하는 구간으로 해석됩니다. 단순히 시즌만 기다리기보다는, 능동적 시장 대응을 함께 고려할 필요가 있습니다.`
            : `현재 부동산 카드 조합은 방향성 탐색 가능성을 시사하는 구간으로 해석됩니다. 단순히 한쪽 방향으로 결단하기보다는, 시장 신호를 함께 살피며 대응할 필요가 있습니다.`;
    } else {
      para1 = signal === "SELL" ? `현재 부동산 카드 조합은 추가 조정 가능성을 시사하는 구간으로 해석됩니다. 단순히 가격만으로 진입을 판단하기보다는, 시장 흐름과 카드 본질 의미를 함께 고려할 필요가 있습니다.`
            : signal === "BUY"  ? `현재 부동산 카드 조합은 매수자에게 우호적 흐름 가능성을 시사하는 구간으로 해석됩니다. 단순히 가격만 보기보다는, 매물 조건과 시장 신호를 함께 점검할 필요가 있습니다.`
            : `현재 부동산 카드 조합은 방향성 탐색 가능성을 시사하는 구간으로 해석됩니다. 단순히 즉각적 결단보다는, 시장 데이터와 흐름을 함께 살필 필요가 있습니다.`;
    }
  } else if (domain === "love") {
    para1 = signal === "SELL" ? `현재 카드 조합은 관계 정체나 재정립 가능성을 시사하는 구간으로 해석됩니다. 단순히 한쪽 방향으로 밀어붙이기보다는, 두 사람의 흐름 변화를 함께 살필 필요가 있습니다.`
          : signal === "BUY"  ? `현재 카드 조합은 관계 진전 가능성을 시사하는 흐름으로 해석됩니다. 단순히 기다리기보다는, 진솔한 표현이 흐름을 만드는 시점일 수 있습니다.`
          : `현재 카드 조합은 관계의 관찰과 인내 시점 가능성을 시사하는 구간으로 해석됩니다. 단순히 결과를 재촉하기보다는, 두 사람의 흐름을 함께 살필 필요가 있습니다.`;
  } else {
    // stock / crypto / general
    if (intent === "sell") {
      para1 = signal === "SELL" ? `현재 카드 조합은 추세 지속보다는 흐름 재평가 가능성을 시사하는 구간으로 해석됩니다. 단순히 '버티면 해결'이라 보기보다는, 상황 변화에 따른 대응 전략을 함께 고려할 필요가 있습니다.`
            : signal === "BUY"  ? `현재 카드 조합은 추세 흐름이 아직 유효할 가능성을 시사하는 구간으로 해석됩니다. 단순히 즉각 결단하기보다는, 흐름의 정점을 함께 살필 필요가 있습니다.`
            : `현재 카드 조합은 방향성 탐색 가능성을 시사하는 구간으로 해석됩니다. 단순히 한쪽 방향으로 결단하기보다는, 흐름과 신호를 함께 점검할 필요가 있습니다.`;
    } else {
      para1 = signal === "SELL" ? `현재 카드 조합은 추가 조정 가능성을 시사하는 구간으로 해석됩니다. 단순히 점수만으로 판단하기보다는, 카드의 본질 의미와 흐름을 함께 고려할 필요가 있습니다.`
            : signal === "BUY"  ? `현재 카드 조합은 진입 흐름이 유효할 가능성을 시사하는 구간으로 해석됩니다. 단순히 한 번에 모든 결정을 내리기보다는, 흐름과 신호를 함께 살필 필요가 있습니다.`
            : `현재 카드 조합은 신호 검증 가능성을 시사하는 구간으로 해석됩니다. 단순히 즉각 행동하기보다는, 흐름과 신호를 함께 살필 필요가 있습니다.`;
    }
  }
  ...
  if (domain === "realestate") {
    if (intent === "sell") {
      para2 = signal === "SELL" ? `호가 조정이나 다음 시즌 대기 같은 유연한 접근이 거래 성사 가능성에 도움이 될 수 있습니다. 특히 호가 집착 상태로 유지할 경우 장기 미거래 가능성도 염두에 둘 필요가 있습니다.`
            : signal === "BUY"  ? `호가 견고 유지나 시즌 적극 활용 같은 능동적 접근이 거래 가치 보존에 도움이 될 수 있습니다. 특히 시즌을 놓칠 경우 다음 기회까지 시간 비용이 늘어날 가능성도 염두에 둘 필요가 있습니다.`
            : `시장 반응 관찰과 조건 유연 운용이 협상력 확보에 도움이 될 수 있습니다. 특히 한쪽 방향만 고집할 경우 거래 기회 손실 가능성도 염두에 둘 필요가 있습니다.`;
    } else {
      para2 = signal === "SELL" ? `저점 신호 확인 후 진입하거나 자금 여력 유지 같은 보수적 접근이 리스크 관리에 도움이 될 수 있습니다. 특히 충동 진입 시 추가 조정 노출 가능성도 염두에 둘 필요가 있습니다.`
            : signal === "BUY"  ? `급매 포착과 신중한 조건 협상 같은 적극적 접근이 가치 확보에 도움이 될 수 있습니다. 특히 시즌 진입을 놓칠 경우 가격 상승 가능성도 염두에 둘 필요가 있습니다.`
            : `데이터 수집과 명확한 신호 대기 같은 신중한 접근이 협상력 확보에 도움이 될 수 있습니다. 특히 즉각 결단 시 시장 정보 부족 가능성도 염두에 둘 필요가 있습니다.`;
    }
  } else if (domain === "love") {
    para2 = signal === "SELL" ? `한 발짝 물러선 관찰이나 자기 시간 확보 같은 접근이 관계 재정립에 도움이 될 수 있습니다. 특히 무리한 압박 시 관계 피로 누적 가능성도 염두에 둘 필요가 있습니다.`
          : signal === "BUY"  ? `진솔한 감정 표현이나 작은 행동 변화 같은 접근이 관계 진전에 도움이 될 수 있습니다. 특히 표현 없이 기다릴 경우 신호가 사라질 가능성도 염두에 둘 필요가 있습니다.`
          : `관찰과 자연스러운 흐름 따라가기가 관계 깊이 형성에 도움이 될 수 있습니다. 특히 결과 재촉 시 흐름이 끊길 가능성도 염두에 둘 필요가 있습니다.`;
  } else {
    // stock / crypto / general
    if (intent === "sell") {
      para2 = signal === "SELL" ? `포지션 일부를 점진적으로 조정하는 접근이나, 손실 제한 기준을 사전에 점검하는 방식이 리스크 관리에 도움이 될 수 있습니다. 특히 흐름 점검 없이 유지할 경우 시간과 추세 양쪽 리스크에 동시에 노출될 가능성도 염두에 둘 필요가 있습니다.`
            : signal === "BUY"  ? `핵심 보유 유지와 분할 익절 준비 같은 접근이 균형 잡힌 대응에 도움이 될 수 있습니다. 특히 단기 변동에 휘둘릴 경우 추세 정점을 놓칠 가능성도 염두에 둘 필요가 있습니다.`
            : `단계적 정리와 신호 검증 같은 접근이 안정적 대응에 도움이 될 수 있습니다. 특히 한쪽 방향으로 결단 시 다른 흐름을 놓칠 가능성도 염두에 둘 필요가 있습니다.`;
    } else {
      para2 = signal === "SELL" ? `진입 시점 재평가나 자금 여력 유지 같은 보수적 접근이 리스크 관리에 도움이 될 수 있습니다. 특히 충동 진입 시 추가 조정 노출 가능성도 염두에 둘 필요가 있습니다.`
            : signal === "BUY"  ? `분할 진입이나 추세 추종 같은 접근이 균형 잡힌 대응에 도움이 될 수 있습니다. 특히 일괄 진입 시 단기 변동성 노출 가능성도 염두에 둘 필요가 있습니다.`
            : `신호 검증과 단계적 진입 같은 접근이 안정적 대응에 도움이 될 수 있습니다. 특히 즉각 결단 시 신호 미확인 가능성도 염두에 둘 필요가 있습니다.`;
    }
  }
  ...
  let para3;
  if (domain === "realestate") {
    para3 = signal === "SELL" && intent === "sell" ? `따라서 현재 구간은 '호가 집착'보다는 👉 시장 변화를 살피며 유연하게 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.`
          : signal === "BUY"  && intent === "sell" ? `따라서 현재 구간은 '관망 유지'보다는 👉 시즌을 활용해 능동적으로 거래를 준비하는 전략이 보다 현실적인 선택지로 해석됩니다.`
          : signal === "SELL" && intent === "buy"  ? `따라서 현재 구간은 '충동 진입'보다는 👉 신호를 확인하며 신중하게 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.`
          : signal === "BUY"  && intent === "buy"  ? `따라서 현재 구간은 '관망 지속'보다는 👉 급매를 살피며 능동적으로 진입을 준비하는 전략이 보다 현실적인 선택지로 해석됩니다.`
          : `따라서 현재 구간은 '한쪽 결단'보다는 👉 시장 신호를 확인하며 유연하게 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.`;
  } else if (domain === "love") {
    para3 = signal === "SELL" ? `따라서 현재 구간은 '강한 압박'보다는 👉 관계 흐름을 살피며 자연스럽게 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.`
          : signal === "BUY"  ? `따라서 현재 구간은 '소극적 기다림'보다는 👉 진솔한 표현으로 흐름을 만드는 전략이 보다 현실적인 선택지로 해석됩니다.`
          : `따라서 현재 구간은 '결과 재촉'보다는 👉 관계 흐름을 자연스럽게 살피는 전략이 보다 현실적인 선택지로 해석됩니다.`;
  } else {
    // stock / crypto / general
    para3 = signal === "SELL" && intent === "sell" ? `따라서 현재 구간은 '무대응 유지'보다는 👉 흐름을 확인하며 유연하게 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.`
          : signal === "BUY"  && intent === "sell" ? `따라서 현재 구간은 '성급한 결단'보다는 👉 추세 정점을 살피며 단계적으로 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.`
          : signal === "SELL" && intent === "buy"  ? `따라서 현재 구간은 '충동 진입'보다는 👉 신호를 확인하며 신중하게 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.`
          : signal === "BUY"  && intent === "buy"  ? `따라서 현재 구간은 '일괄 진입'보다는 👉 분할 접근으로 흐름과 함께 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.`
          : `따라서 현재 구간은 '즉각 결단'보다는 👉 신호와 흐름을 함께 점검하며 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.`;
  }

  // ─ 마지막 면책 (도메인별 통일) ─
  const disclaimer = (domain === "stock" || domain === "crypto")
    ? `※ 본 해석은 흐름에 대한 참고 관점이며, 실제 판단은 시장 상황과 함께 종합적으로 고려하시기 바랍니다.`
    : domain === "realestate"
    ? `※ 본 해석은 흐름에 대한 참고 관점이며, 실제 판단은 시장 상황과 전문가 자문을 종합적으로 고려하시기 바랍니다.`
    : domain === "love"
    ? `※ 본 해석은 흐름에 대한 참고 관점이며, 실제 관계는 두 사람의 진심을 종합적으로 고려하시기 바랍니다.`
    : `※ 본 해석은 흐름에 대한 참고 관점이며, 실제 판단은 본인의 상황을 종합적으로 고려하시기 바랍니다.`;
...
function buildStockMetrics({ totalScore, riskScore, cleanCards, isLeverage, queryType, prompt, intent, reversedFlags }) {
  ...
  if (_uncOverride) {
    // [V24.7+V25.9~V25.9.3] sell intent + 게이트 발동 시
    //   사장님 최종안 (V25.9.3): 3-paragraph + 👉 강조 + ※ 마지막 면책
    //   법무 안전성 95% → 99% 강화
    if (stockIntent === 'sell') {
      const _isStrongSell = totalScore <= -3 || volGate.isHighVolatility || volGate.hasExtremeCard;
      // 사장님 최종안 — 3단락 + 👉 + ※ 면책
      criticalInterpretation = _isStrongSell
        ? `현재 카드 조합은 추세 지속보다는 흐름 재평가 가능성을 시사하는 구간으로 해석됩니다. 단순히 '버티면 해결'이라 보기보다는, 상황 변화에 따른 대응 전략을 함께 고려할 필요가 있습니다.\n\n포지션 일부를 점진적으로 조정하는 접근이나, 손실 제한 기준을 사전에 점검하는 방식이 리스크 관리에 도움이 될 수 있습니다. 특히 흐름 점검 없이 유지할 경우 시간과 추세 양쪽 리스크에 동시에 노출될 가능성도 염두에 둘 필요가 있습니다.\n\n따라서 현재 구간은 '무대응 유지'보다는 👉 흐름을 확인하며 유연하게 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.\n\n※ 본 해석은 흐름에 대한 참고 관점이며, 실제 판단은 시장 상황과 함께 종합적으로 고려하시기 바랍니다.`
        : `현재 카드 조합은 회복 신뢰도가 낮은 구간으로 흐름 재평가 가능성을 시사하는 시점으로 해석됩니다. 단순히 결과를 기다리기보다는, 상황 변화에 따른 대응 전략을 함께 고려할 필요가 있습니다.\n\n포지션 일부를 점진적으로 조정하는 접근이나, 단기 흐름 회복 시점에서의 보수적 대응이 균형 잡힌 선택지로 해석될 수 있습니다. 무대응도 결단도 한쪽으로 치우친 선택일 가능성을 염두에 둘 필요가 있습니다.\n\n따라서 현재 구간은 '무대응 유지'보다는 👉 흐름을 확인하며 유연하게 대응하는 전략이 보다 현실적인 선택지로 해석됩니다.\n\n※ 본 해석은 흐름에 대한 참고 관점이며, 실제 판단은 시장 상황과 함께 종합적으로 고려하시기 바랍니다.`;
    } else {
      // buy intent — 동일한 사장님 최종안 패턴
      ...
      criticalInterpretation = `${_gateGeneralMsg} 단순히 점수만으로 판단하기보다는, 카드의 본질 의미와 게이트 발동 사실을 함께 고려할 필요가 있습니다.\n\n객관적 신호(거래량·이평선·추세) 확인 전 신규 진입 보류가 보수적 접근으로 해석될 수 있습니다. 점수가 긍정 시그널처럼 보이더라도, 카드의 본질 의미가 다른 방향을 가리킬 가능성을 염두에 둘 필요가 있습니다.\n\n따라서 현재 구간은 '점수 추종'보다는 👉 카드의 본질 의미와 흐름을 함께 점검하는 전략이 보다 현실적인 선택지로 해석됩니다.\n\n※ 본 해석은 흐름에 대한 참고 관점이며, 실제 판단은 시장 상황과 함께 종합적으로 고려하시기 바랍니다.`;
    }
  }
