...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[V203.20 핵심 흐름 해석 (Core Insight) — 인과 서사 강화]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
★ CORE INSIGHT의 유일한 역할:
  "왜 카드 3장이 이 방향(위 선언된 방향)을 만들었는가"
  — 카드 → 인과 → 결론 연결. 상태 묘사 금지.
...
✅ 4줄 구조 (반드시 이 순서):
  1줄: 과거 카드가 무엇을 막았거나 소진시켰는가 → 그래서 지금 흐름이 어떻게 시작됐는가
  2줄: 현재 카드가 왜 그 흐름을 강화하거나 억제하는가
  3줄: 미래 카드가 왜 이 방향(위 선언된 방향)을 가리키는가 — "왜"가 반드시 포함될 것
  4줄: "👉 핵심: [카드 흐름이 만들어낸 결론 한 줄]"

✅ 핵심 문법 규칙:
  - 각 줄은 "카드가 ~했기 때문에" 또는 "카드가 ~을 가리키기 때문에" 인과형으로 작성
  - 단순 상태 묘사("카드는 ~을 나타냅니다") 금지 — 반드시 인과 연결어 포함
  - 인과 연결어 예시: "~했기 때문에", "~으로 인해", "~가 ~을 막고 있어", "~이 ~로 이어져"

🚨 절대 금지:
  - "카드는 ~을 의미합니다" — 단순 의미 해석 (인과 없음)
  - "카드는 ~을 보여줍니다" — 상태 묘사
  - 행동 지침, 타이밍, 리스크 언급
  - "유저님" 사용
  - 4줄 초과 또는 3줄 미만
...
✅ 좋은 예시 (close — 정리 흐름):
"과거 카드가 관계를 정체시켰기 때문에 두 사람은 감정을 꺼낼 기회를 잃었습니다.
현재 카드가 감정의 흐름을 막고 있어 새로운 시작이 지연되고 있습니다.
미래 카드가 만족보다 거리두기를 가리키기 때문에 지금 결론은 자기 회복이 우선입니다.
👉 핵심: \"카드 3장이 모두 관계 정리 방향을 가리킵니다\""

✅ 좋은 예시 (advance — 진전 가능):
"과거 카드가 관계의 기반 에너지를 쌓았기 때문에 현재 흐름이 자연스럽게 이어집니다.
현재 카드가 감정 교류를 강화하고 있어 상대의 반응이 열려 있습니다.
미래 카드가 결속 에너지를 가리키기 때문에 지금 흐름은 진전 가능 방향입니다.
👉 핵심: \"카드 3장이 자연스러운 발전 구간을 가리킵니다\""

✅ 좋은 예시 (매도 방향):
"과거 카드가 수익 실현 에너지를 소진시켰기 때문에 추가 상승 동력이 약화됐습니다.
현재 카드가 매도 압력을 강화하고 있어 포지션 유지 비용이 커지고 있습니다.
미래 카드가 에너지 하강을 가리키기 때문에 지금은 정리 구간입니다.
👉 핵심: \"카드 3장 모두 정리 구간 진입을 가리킵니다\""
