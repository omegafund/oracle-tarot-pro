  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/verify-payment" && request.method === "POST") {
      try {
        ...
        // ── 테스트 모드: "TEST-PAY-2900" 이면 통과
        // ── 실제 운영 시: 토스페이먼츠 등 PG API 호출로 교체
...
안녕하세요, 유저님. (질문에 대한 짧고 정중한 인사 한 줄)

과거
과거의 흐름은 (카드 이미지 묘사로 시작하여 당시 상황을 서술형 단락으로 작성)

현재
현재의 흐름은 (카드 이미지 묘사로 시작하여 지금의 에너지를 서술형 단락으로 작성)

미래
미래의 흐름은 (카드 이미지 묘사로 시작하여 다가올 가능성을 서술형 단락으로 작성)

제우스의 최종신탁 (ZEUS SUPREME ORACLE)
(전체 리딩을 통합하는 최종 결론 서술)
...
규칙:
- "과거", "현재", "미래" 단어는 반드시 한 줄에 단독으로 출력.
- "제우스의 최종신탁 (ZEUS SUPREME ORACLE)" 명칭을 정확히 출력.
- 모든 섹션의 시작은 "~의 흐름은"으로 자연스럽게 시작할 것.
`;
